...
В данный момент доступны следующие мтеоды:
...
| ```existInNluTokens(...$values)```                       | Возвращает true, если  хотя бы одно слово из аргументов находится в nluTokens |
...
| ```setResponseSessionState(string $key, mixed $value)``` | Устанавливает значение $value по ключу $key в массив состояний сессии         |
...
| ```setResponseUserState(string $key, mixed $value)```    | Устанавливает значение $value по ключу $key в массив состояний пользователя   |
...
| ```setPush(string $text, array $payload)```              | Устанавливает пуш уведомление с текстом $text и нагрузкой $payload (array)    |
...
| Тип                                            | Условия                                                      |
|------------------------------------------------|--------------------------------------------------------------|
| VKMarLib\Exceptions\MarusiaResponseException   | Возникает при ошибке формировании ответа для Маруси          |
| VKMarLib\Exceptions\MarusiaRequestException    | Возникает при ошибке чтения запроса от Маруси                |
| VKMarLib\Exceptions\MarusiaValidationException | Возникает при нарушении валидации данных в ответе для Маруси |
